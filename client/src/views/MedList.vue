<!-- <template>
    <v-container>
        <h1>Med Page</h1>
        <br>
        <v-btn  @click="deleteSelected" v-if="posts.length">Delete Selected</v-btn>
        <v-row>
...
<template>
    <v-container>
      <h1 class="display-4">Medications</h1>
      <br>
      <v-btn v-if="posts.length" @click="deleteSelected">Delete Selected</v-btn>
      <v-row>
        <v-col sm="4" class="pa-3 mt-3" v-for="post in posts" :key="post._id">
          <v-card class="pa-1">
            <v-card-title>
            ...
            <v-card-subtitle>
              ...
              <v-checkbox v-model="selectedItems" :value="post._id" label="Select for deletion"></v-checkbox>
            </v-card-subtitle>
            <v-btn class="ml-4 mt-3 mb-3" small variant="outlined" color="indigo"
              :to="{ name: 'med', params: { id: post._id }}">
              Open
